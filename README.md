...
A production-ready REST API that fetches GitHub user profiles via the GitHub Public API, computes useful insights, stores them in MySQL, and exposes clean endpoints for querying the data.
...
**Example:**
```bash
curl "http://localhost:3000/api/profiles?sort=total_stars&limit=5"
```
...
```json
{
  "success": true,
  "data": [
    { "language": "JavaScript", "repo_count": 28, "percentage": 45.90 },
    { "language": "Python",     "repo_count": 18, "percentage": 29.51 },
    { "language": "TypeScript", "repo_count": 15, "percentage": 24.59 }
  ]
}
```
...
```
score = log1p(followers) × 3 + log1p(total_stars) × 4 + log1p(public_repos) × 2
```
...
Always set `GITHUB_TOKEN` in production.
